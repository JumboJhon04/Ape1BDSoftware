...
function EstudianteInventoryPage() {
  ...
  const filteredArticulos = useMemo(() => {
    return articulos.filter((item) => {
      ...
      if (normalizeText(item.estado) !== 'disponible') return false

      const matchesCategoria =
        appliedFilters.categoria === 'Todas' ||
        normalizeText(item.categoria) === normalizeText(appliedFilters.categoria)

      const matchesSearch =
        !appliedFilters.search ||
        normalizeText(item.nombre).includes(normalizeText(appliedFilters.search)) ||
        normalizeText(item.codigoInstitucional).includes(normalizeText(appliedFilters.search))
  ...
  const toggleSelection = (item) => {
    ...
    setSelectedArticulos((prev) => {
      ...
      if (exists) return prev.filter((a) => (a.idArticulo ?? a.IdArticulo) !== id)
      return [...prev, item]
    })
  }
  ...
  const removeItemFromCart = (idToRemove) => {
    setSelectedArticulos((prev) => {
      ...
      if (updated.length === 0 && showModal) closeModal()
      return updated
    })
  }
        ...
        {loading ? (
          ...
          <div className="docente-catalog-grid">
            {filteredArticulos.map((item) => {
              ...
              return (
                <article
                  key={item.idArticulo ?? item.IdArticulo}
                  className={`docente-catalog-card ${selected ? 'selected-card' : ''}`}
                >
          ...
          >
            <header className="modal-header premium-modal-header">
              <h2 id="solicitar-equipo-title">
                {solicitarSuccess
                  ? '¡Solicitud Exitosa!'
                  : `Resumen de Solicitud (${selectedArticulos.length} ${selectedArticulos.length === 1 ? 'equipo' : 'equipos'})`}
              </h2>
                ...
                <>
                  ...
                  <form onSubmit={handleSolicitar} className="docente-modal-form mt-4">
                    <div className="docente-modal-form-group p-4 bg-gray-50 border rounded-md">
                      <label htmlFor="fecha-devolucion" className="docente-modal-label text-primary font-semibold">
                        Fecha de Devolución para el Lote
                      </label>
                      <p className="text-sm text-gray-500 mb-2">
                        Selecciona cuándo devolverás todos los equipos listados.
                      </p>
                      <input
                        id="fecha-devolucion"
                        type="date"
...
export default EstudianteInventoryPage